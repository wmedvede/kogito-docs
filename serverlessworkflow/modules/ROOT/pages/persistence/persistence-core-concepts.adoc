...
The SonataFlow workflows persistence is the mechanism to ensure that your workflow instances remains consistent in the context of an error or a runtime restart. For example, a pod restart, a programmed maintenance shutdown, etc.
...
2. Add other quarkus extensions that usually depends on the selected database type
...
* xref:persistence/persistence-with-postgresql.adoc[{product_name} Persistence with PostreSQL]
