...
.List of parameters for the Kafka eventing configuration
[cols="2,1,1"]
|===
|Variable | Description| Default value

|QUARKUS_PROFILE
|Set the quarkus profile with the value `events_support` to enable the kafka messaging based Job Service Eventing API.
...
|`localhost:9092` when the `events-support` profile is set.
...
|`kogito-job-service-job-request-events-v2` when the `events_support` profile is set.
...
|`kogito-jobs-events` when the `events_support` profile is set.
